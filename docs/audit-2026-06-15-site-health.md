...
>
> **RESOLVED & SHIPPED 2026-06-16 (merged to main).** Root cause for the money pages turned out simpler than feared: in Next 16 a `[slug]` route renders on-demand DYNAMIC by default (`no-store`) unless opted into static. A `dynamic='error'` build proved compare/best/blog use **zero dynamic API** → `export const dynamic = 'force-static'` makes them edge-cached ISR. No risky `unstable_cache`/tag wiring needed — the freshness cascade's existing `revalidatePath` invalidates, with `revalidate=3600` as the self-healing backstop. **Verified:** compare `MISS→HIT` `s-maxage=3600` (local prod build), best/blog prerendered `●`; logged-in + admin verified on the Vercel preview. Layer 1 (client-side auth) shipped with it.
>
> **LAYER 3 ALSO SHIPPED 2026-06-16 — tool pages (the #1 traffic surface).** They read cookies for per-user saved/already-reviewed state + `SentimentSynthesis` used the cookie client. Moved per-user state to a no-store `/api/me/tool-state/[toolId]` (fetched client-side by SaveToolButton + QuickFeedback), switched SentimentSynthesis to the admin client, and `force-static`. Verified live: `x-vercel-cache: MISS→HIT`, `x-nextjs-prerender: 1`; logged-in save/unsave persists. **All money pages (compare, best, blog, tools) are now edge-cached.** Remaining dynamic-by-design: homepage (recently-viewed cookie) + auth-gated routes (admin/dashboard) — correct to leave dynamic.
