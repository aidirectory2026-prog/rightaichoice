...
### #6 Measurement layer — niche-page tracker (views + dashboard + digest) (`1a8d1b2` + digest follow-up)
Closes the loop on "build → measure → deepen winners." Three service-role-only Postgres objects: **`tracked_niche_pages`** (the 64 niche slugs, seeded straight from `lib/data/best-pages.ts` so it can't drift), **`niche_page_metrics`** (per-page 28-day time series, unnested from `gsc_snapshots`, impression-weighted avg position), and **`niche_page_latest`** (latest snapshot + Δ-vs-prior, impressions-desc). All `revoke anon, authenticated` / `grant service_role` — GSC data is not publicly exposed. **`/admin/niche-tracker`** (new admin-nav link) reads them via the admin client: summary tiles (tracked · with-data · gaining · total impr/clicks) + a sorted "pages with impressions" table (niche → live page, impr, Δ, avg pos, clicks, queries) + "awaiting first impressions" chips. **Weekly digest integration:** `email-weekly-digest` (Mon 08:00 UTC) now appends a "Niche pages" strip — `with-data/total · impr · clicks` + the **top-5 week-over-week impression gainers** + a deep-link — so the operator sees warming niches without opening the dashboard. Cadence: refreshes weekly with the GSC snapshot; pages built recently read 0 for ~2–4 weeks (pre-index). Baseline at ship: **12 niches with impressions** — accounting 145 (+46), education 59 (+20), game-dev 27 (+8) — all pos 53–86 → 0 clicks (authority-gated, doc 20).
