...
### 2026-05-31 — Gold-standard GATED new-tool SOP (Workstream C)
- **What:** Every new tool now runs a 10-step gated pipeline and only PUBLISHES when it meets the standard of our best pages — with a per-tool QA scorecard.
- **Why:** "Any new tool goes through high-standard SOPs with all sections present." Day-0 tools used to publish thin; now they're drafts until QA-green.
- **How (non-technical):** A new tool starts hidden (draft). The robot fills it step by step — editorial verdict, category, viability, latest news, logo, alternatives, 2+ comparison pages, 9+ FAQs, sentiment — and checks each step. Hard checks must pass or it stays hidden; we can see exactly which check failed on `/admin/onboarding`. Only when everything required is green does it go live (and get pinged to IndexNow).
- **How (technical):** Extends A4's `lib/cron/onboard.ts` → `onboardOne()` (10 steps) + `runOnboardSop({slugs})` draft lane. New: `lib/cron/logo.ts` (shared resolve→verify→bucket logo, also used by `scripts/backfill-logos.ts`), `lib/cron/onboard-compares.ts` (creates ≤3 `is_editorial` `tool_comparisons` vs top alternatives + `generateCompareEditorial()` extracted from cascade), `lib/cron/onboard-faqs.ts` (`generateLongTailFaqs` → ≥9 `faqs_long_tail`). `refresh.ts` gained `includeUnpublished` so drafts refresh. Per-tool `tool_onboarding_qa` (migration 134: tool_id PK→tools, checks jsonb, all_green, published; RLS service-only). **HARD gates** (block publish): description ≥300, ≥3 features, valid pricing_type, ≥1 category, ≥3 alternatives (live via `getAlternativeTools` — no edge table), ≥2 editorial compares, ≥9 FAQs, best_for/not_for/editorial_verdict. **SOFT** (warn): logo (favicon ok), sentiment, tutorials, models, latest_updates. Publish flips `is_published` only when all HARD green, then invokes `submitToIndexNow('/tools/<slug>')` (SEO files untouched). `/admin/onboarding` shows drafts pending + failing-gate tools. Smoke-tested on a throwaway draft (correct fail→no-publish) then deleted.
- **⚠️ Operational dependency:** enrichment/compares/FAQs require **DeepSeek + Anthropic credit**; the smoke test hit DeepSeek 402 / Anthropic low-credit — top up before bulk onboarding (D2).
