-- 164 (Phase 10.7e.2, 2026-06-13) — rollup-vs-raw reconciliation invariant.
--
-- Verification-mean #5 (plan §5). Adds I13 to the nightly tracking_health
-- suite: recompute the same aggregate from RAW user_events and from the 7e
-- ROLLUP tables for a recent window and assert they match exactly. Two legs:
--   I13a — daily event counts (event_rollup_daily summed back to per-day).
--   I13b — distinct visitors (dau_rollup_daily vs raw distinct distinct_id).
--
-- TOLERANCE / CURRENT-HOUR HANDLING: the rollup's hourly cron lags raw by up
-- to an hour, and the current partial IST day is still accumulating, so a
-- naive same-instant compare would false-alarm. We therefore compare ONLY
-- COMPLETE IST DAYS strictly BEFORE today (day_ist < today_ist). Those days
-- are immutable, the hourly job has long since converged on them, and the
-- match must be EXACT (tolerance = 0 rows). A divergence => status 'fail'.
--
-- The function is rebuilt FROM ITS LIVE DEFINITION (pulled 2026-06-13);
-- I1–I12 are byte-identical to the deployed body, I13a/I13b are appended.
-- run_tracking_invariants() has a fixed no-arg signature so CREATE OR REPLACE
-- is in-place (no overload risk).
...
declare
  ...
  v_orphan_list  text;
  v_dup_7d       bigint;
  -- 10.7e.2 additions (rollup reconciliation)
  v_recon_evt    bigint;
  v_recon_dau    bigint;
  ...
  -- 10.7e.2 — refreshed to the live 101-name registry (the deployed array
  -- predated the 7b/7c merge, so I11 false-failed on error_encountered +
  -- web_vitals which ARE schema'd). Also dropped two PLANNED/demoted names
  -- (activation_milestone, recommendation_requested) that are no longer
  -- schema keys.
...
begin
  ...
  -- ── I13a: rollup-vs-raw daily EVENT COUNT reconciliation (10.7e.2) ──
  -- Compare per-IST-day total event counts from event_rollup_daily against a
  -- fresh raw aggregation, for COMPLETE days in the last 30 (day_ist < today,
  -- so the still-accumulating current day and the <=1h cron lag never alarm).
  -- v_recon_evt = number of days that DIVERGE. Must be 0 (exact, tolerance 0).
  ...
  -- ── I13b: rollup-vs-raw daily DAU reconciliation (10.7e.2) ──────────
  -- Same window/exclusion, comparing dau_rollup_daily distinct visitors per
  -- (day, bot flag) against raw count(distinct distinct_id).
