...
import type { SentimentReport as Report, CachedSentiment } from '@/lib/data/sentiment'
...
export function SentimentReportPage({ toolSlug, toolName, cached }: { toolSlug: string; toolName: string; cached?: CachedSentiment | null }) {
  const { user } = useAuth()
  // StackBack Phase A: the latest shared cached report is the public default
  // (no login). A fresh real-time scan is the gated/paid action that replaces it.
  const [phase, setPhase] = useState<Phase>(cached ? 'report' : 'idle')
  const [status, setStatus] = useState<Status | null>(null)
  const [report, setReport] = useState<Report | null>(cached?.report ?? null)
  const [resultMeta, setResultMeta] = useState<{ sources: string[]; mention_count: number } | null>(
    cached ? { sources: cached.sources, mention_count: cached.mention_count } : null,
  )
  const [cachedView, setCachedView] = useState(!!cached)
  const cachedAt = cached?.scraped_at ?? null
  ...
  useEffect(() => {
    ...
    if (cached) analytics.sentimentResultViewed(toolSlug, cached.report.sentiment_score, 'cached')
    // eslint-disable-next-line react-hooks/set-state-in-effect
    void refreshStatus()
  }, [toolSlug, refreshStatus, cached])
        ...
        {(phase === 'idle' || phase === 'report') && (
          <div className="relative mt-6 flex flex-wrap items-center gap-3">
            <button onClick={runScan} className="group inline-flex items-center gap-2 rounded-xl bg-emerald-500 px-6 py-3 text-sm font-bold text-emerald-950 shadow-lg shadow-emerald-500/20 transition hover:bg-emerald-400">
              ...
              {!user ? `Sign up free — unlock ${freeLimit} scans` : phase === 'report' ? 'Run a fresh real-time scan' : freeLeft !== null && freeLeft <= 0 ? `Run a scan — ${price}` : 'Run my free sentiment scan'}
          ...
          {cachedView && (
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-zinc-800 bg-zinc-900/50 px-4 py-3">
              <p className="text-xs text-zinc-400">
                <span className="font-medium text-zinc-300">Latest community read</span>
                {cachedAt ? ` · as of ${timeAgo(cachedAt)}` : ''} · shared snapshot of {toolName}
              </p>
              <button onClick={runScan} className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-300 hover:text-emerald-200">
                {!user ? <Lock className="h-3.5 w-3.5" /> : <Zap className="h-3.5 w-3.5" />}
                Run a fresh real-time scan
                <ArrowRight className="h-3.5 w-3.5" />
              </button>
            </div>
          )}
          <ReportBody report={report} toolName={toolName} meta={resultMeta} onDownload={download} />
        </>
      )}
