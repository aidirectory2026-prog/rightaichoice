-- Rollback for 183_filter_honesty.sql — restores the pre-183 definitions
-- (captured verbatim from the live DB via pg_get_functiondef on 2026-07-02).

-- ── 1. insights_search_log ──────────────────────────────────────────────
drop function if exists public.insights_search_log(timestamptz, timestamptz, boolean, integer, jsonb);

create function public.insights_search_log(p_cutoff timestamptz, p_end timestamptz default null, p_include_bots boolean default false, p_limit integer default 200)
...
revoke all on function public.insights_search_log(timestamptz, timestamptz, boolean, integer) from public, anon, authenticated;
grant execute on function public.insights_search_log(timestamptz, timestamptz, boolean, integer) to service_role;

-- ── 2. insights_plan_dropoff ────────────────────────────────────────────
drop function if exists public.insights_plan_dropoff(timestamptz, timestamptz, boolean, jsonb);

create function public.insights_plan_dropoff(p_cutoff timestamptz, p_end timestamptz default null, p_include_bots boolean default false)
...
revoke all on function public.insights_plan_dropoff(timestamptz, timestamptz, boolean) from public, anon, authenticated;
grant execute on function public.insights_plan_dropoff(timestamptz, timestamptz, boolean) to service_role;

-- ── 3. insights_error_overview ──────────────────────────────────────────
drop function if exists public.insights_error_overview(timestamptz, timestamptz, boolean, jsonb);

create function public.insights_error_overview(p_cutoff timestamptz, p_end timestamptz default null, p_include_bots boolean default false)
...
revoke all on function public.insights_error_overview(timestamptz, timestamptz, boolean) from public, anon, authenticated;
grant execute on function public.insights_error_overview(timestamptz, timestamptz, boolean) to service_role;

-- ── 4. insights_live_sessions ───────────────────────────────────────────
drop function if exists public.insights_live_sessions(integer, boolean, jsonb);

create function public.insights_live_sessions(p_active_within_sec integer default 300, p_include_bots boolean default false)
...
revoke all on function public.insights_live_sessions(integer, boolean) from public, anon;
grant execute on function public.insights_live_sessions(integer, boolean) to service_role, authenticated;

-- ── 5. insights_activity_feed ───────────────────────────────────────────
drop function if exists public.insights_activity_feed(integer, boolean, jsonb);

create function public.insights_activity_feed(p_limit integer default 30, p_include_bots boolean default false)
...
revoke all on function public.insights_activity_feed(integer, boolean) from public, anon, authenticated;
grant execute on function public.insights_activity_feed(integer, boolean) to service_role;

-- ── 6. insights_user_sessions_v2 — restore 158_user360_v2.sql shape ─────
drop function if exists public.insights_user_sessions_v2(text, integer, integer, timestamptz, timestamptz, text[]);
...
-- ── 7. insights_user_directory — restore 159_user360_identity.sql shape ─
drop function if exists public.insights_user_directory(timestamptz, timestamptz, boolean, jsonb, text, integer, integer, text);
